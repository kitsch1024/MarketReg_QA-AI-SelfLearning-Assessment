...
def main() -> None:
    ...
    if has_items_hdr:
        ...
        with cA:
            top_start_btn = st.button("Start Session", key="top_start_session")
        with cB:
            top_view_btn = st.button("View Records", key="top_view_records")
        with cC:
            top_load_btn = st.button("Load Progress", key="top_load_progress")
        with cD:
            top_save_btn = st.button("Save Progress", key="top_save_progress")
    ...
    # 顶端按钮与侧边栏按钮合并（任一触发即生效）
    start_btn = bool(start_btn or top_start_btn)
