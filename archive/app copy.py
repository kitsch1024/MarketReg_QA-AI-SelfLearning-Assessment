...
def _badge(label: str, value: str) -> str:
    return (
        "<span style='display:inline-block;padding:2px 8px;margin:2px 6px 6px 0;"
        "border-radius:12px;background:#eef2f6;color:#1f2d3d;font-size:12px;line-height:18px;'>"
...
def render_learning_records() -> None:
    ...
    st.markdown(
        """
        <style>
        .stCard{padding:12px 16px;border:1px solid #eef2f7;border-radius:12px;margin-bottom:14px;background:#ffffff;box-shadow:0 1px 2px rgba(16,24,40,0.04)}
        .badge{display:inline-block;background:#f8fafc;color:#334155;padding:2px 8px;border-radius:999px;font-size:12px;margin-right:6px;margin-bottom:4px}
        .kpi{font-size:13px;color:#64748b}
        .kpi-strong{font-size:18px;font-weight:600;color:#111827}
        .section-title{font-weight:700;font-size:18px;margin:4px 0 10px;color:#111827}
        .softDivider{height:1px;margin:14px 0;background:linear-gradient(90deg, rgba(0,0,0,0) 0%, #e5e7eb 20%, #e5e7eb 80%, rgba(0,0,0,0) 100%)}
    ...
    try:
        ...
        try:
            ...
            line = alt.Chart(df).mark_line(point=True).encode(
                ...
                y=alt.Y('accuracy_pct:Q', title='accuracy_pct(%)'),
                tooltip=['time','accuracy_pct']
            ).properties(height=220)
            bar = alt.Chart(df).mark_bar(opacity=0.4).encode(
                x=alt.X('time:N', axis=x_axis),
                y=alt.Y('answered:Q', title='Completed'),
    ...
    for r in data_sorted:
        ...
        if isinstance(filt, dict):
            try:
                ...
                if stds:
                    badges.append(_badge("Source of regulatory documents", ", ".join(stds)))
                if badges:
                    st.markdown(" ".join(badges), unsafe_allow_html=True)
                st.caption("Filters")
                try:
...
def render_round_summary() -> None:
    ...
    st.markdown(
        """
        <style>
        .sumCard{padding:14px 18px;border:1px solid #eef2f7;border-radius:12px;margin:14px 0;background:#ffffff;box-shadow:0 1px 2px rgba(16,24,40,0.04)}
        .sectionTitle{font-weight:700;font-size:16px;margin:0 0 10px;color:#111827}
        .softDivider{height:1px;margin:14px 0;background:linear-gradient(90deg, rgba(0,0,0,0) 0%, #e5e7eb 20%, #e5e7eb 80%, rgba(0,0,0,0) 100%)}
        </style>
        """,
        unsafe_allow_html=True,
...
def main() -> None:
    ...
    if items:
        ...
        st.info(" Enter dataset path at left, scan, then start session.")
