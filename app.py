...
def _infer_internal_type(raw_type: str, choices: Optional[List[str]]) -> str:
    ...
    # Remove common suffixes like "question" for normalization
    # This handles variants like "Single Choice Question", "multiple-choice question", etc.
    t_clean = t_l
    for suffix in [" question", "-question", "_question"]:
        if t_clean.endswith(suffix):
            t_clean = t_clean[:-len(suffix)].strip()
    
    # Map common English labels (handle spaces, slashes, hyphens)
    # Normalize by replacing spaces and hyphens with underscores, removing slashes
    t_normalized = t_clean.replace(" ", "_").replace("-", "_").replace("/", "_")
    if t_normalized in {"single_choice", "multiple_choice", "true_false", "fill_blank", "fill_in_the_blank", "short_answer", "case_analysis"}:
        # Map fill_in_the_blank variants to fill_blank for consistency
        if t_normalized in {"fill_in_the_blank", "fill_in_blank"}:
            return "fill_blank"
        return t_normalized if "_" in t_normalized else "single_choice"
    
    # Additional explicit English mappings with pattern matching
    # Handle "single choice" and all its variants
    if "single" in t_clean and "choice" in t_clean:
        return "single_choice"
    # Handle "multiple choice" and all its variants
    if "multiple" in t_clean and "choice" in t_clean:
        return "multiple_choice"
    # Handle true/false variants
    if t_clean in {"true/false", "true_false", "true-false"}:
        return "true_false"
    # Handle fill-in-blank variants
    if ("fill" in t_clean and "blank" in t_clean) or t_clean in {"fill in the blank", "fill-in-the-blank", "fill in blank", "fill blank"}:
        return "fill_blank"
...
def main() -> None:
    ...
    default_path = os.path.join(os.path.dirname(__file__), "data", "MarketReg_QA.jsonl")
